...
Future<void> generatePDFSedimen(
    //Data Pribadi
    String Nama,
    String NRP,
    String NIP,
  ...
  document.dispose();
  //Launch file.
  await saveAndLaunchFile(bytes, "Sedimen_"+Nama+'.pdf');
}
...
void _drawGridImmunoserologi(PdfPage page,String hbsag,String hiv,String vdrl){
  final Size pageSize = page.getClientSize();
  //-------------------------------------------------------
  //HEMATOLGI
  ...
  if(!hiv.isEmpty || hiv !=""){
    ListData.add(ModelTwo(nama: "HIV", data: hiv));
  }
  if(!vdrl.isEmpty || vdrl !=""){
    ListData.add(ModelTwo(nama: "VDRL", data: vdrl));
  }
...
void _drawGridSedimen(
  ...
  if(!Epitel.isEmpty || Epitel !=""){
    ListData.add(ModelTwo(nama: "Epitel", data: Epitel));
  }
  if(!Lekosit.isEmpty || Lekosit !=""){
    ListData.add(ModelTwo(nama: "Lekosit", data: Lekosit));
  }
  if(!Eritrosit.isEmpty || Eritrosit !=""){
    ListData.add(ModelTwo(nama: "Eritrosit", data: Eritrosit));
  }
  if(!Bakteri.isEmpty || Bakteri !=""){
    ListData.add(ModelTwo(nama: "Bakteri", data: Bakteri));
  }
  if(!Kristal.isEmpty || Kristal !=""){
    ListData.add(ModelTwo(nama: "Kristal", data: Kristal));
  }
  double TrueSize = MainSize+105;
  double Range = 25;
  for(int i=0;i<ListData.length;i++){
